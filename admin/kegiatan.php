...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if ($_SESSION['admin_role'] != 'admin') {
        $check = mysqli_query($koneksi, "SELECT created_by FROM kegiatan WHERE id='$id'");
    ...
    $query_img = mysqli_query($koneksi, "SELECT gambar FROM kegiatan WHERE id='$id'");
...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6">
    <h3 class="text-2xl font-bold text-gray-800 mb-4 sm:mb-0">Kelola Kegiatan</h3>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 text-gray-700 uppercase text-xs font-bold tracking-wider">
                    ...
                    <th class="p-4 border-b border-gray-200">Nama Kegiatan</th>
                    ...
                    <th class="p-4 border-b border-gray-200 w-40">Lokasi</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                $limit_sql = "";
                if ($_SESSION['admin_role'] != 'admin') {
                    $limit_sql = "WHERE created_by='" . $_SESSION['admin_id'] . "'";
                }
                $query = mysqli_query($koneksi, "SELECT * FROM kegiatan $limit_sql ORDER BY tanggal DESC");
                while ($row = mysqli_fetch_assoc($query)):
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <?php
                        if ($row['kategori'] == 'Workshop')
                            echo 'bg-blue-100 text-blue-800';
                        elseif ($row['kategori'] == 'Seminar')
                        ...
                        <td class="p-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                <?php if ($_SESSION['admin_role'] == 'admin' || $row['created_by'] == $_SESSION['admin_id']): ?>
                                    <a href="kegiatan_form.php?id=<?= $row['id'] ?>"
                                    ...
                                    <a href="#"
                                        onclick="event.preventDefault(); confirmDelete('kegiatan.php?action=delete&id=<?= $row['id'] ?>')"
    ...
    <?php if (mysqli_num_rows($query) == 0): ?>
        <div class="p-8 text-center text-gray-500">
            Belum ada data kegiatan. Silahkan tambah data baru.
        </div>
    <?php endif; ?>
</div>
...
<script>
    function copyToClipboard(slug) {
        ...
        const fullUrl = `${baseUrl}/kegiatan_detail.php?slug=${slug}`;
